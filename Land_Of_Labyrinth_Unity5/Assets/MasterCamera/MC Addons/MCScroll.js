			...
			if ( mc.scroller.transform.localPosition.z > (meshHiding.MeshHideDistance*-1)){	
				if(meshHiding.hideAllChildMesh){
					...
					for(var ir = 0 ; ir < rends.Length ; ir ++){
						rends[ir].GetComponent.<Renderer>().enabled = false;
					}
				} else {
					meshHiding.meshObject.GetComponent(Renderer).GetComponent.<Renderer>().enabled = false;
				}
			} else {
				...
				if(meshHiding.hideAllChildMesh){
					...
					for(var irx = 0 ; irx < rends.Length ; irx ++){
						rends[irx].GetComponent.<Renderer>().enabled = true;
					}
				} else {
					meshHiding.meshObject.GetComponent(Renderer).GetComponent.<Renderer>().enabled = true;
				}
